...
const Email = ({ buyerName, sellerName, totalPrice, trip, purchaseId, orderNumber, bookingRef, bookingName }: Props) => {
  ...
  return (
    <EmailLayout preview="Your swappup purchase is confirmed" transactional>
      ...
      <Text style={p}>
        Your money is safe with us and will only be sent to {seller} once your name is on the booking.
      </Text>
      ...
      <Text style={p} as="div">
        <ol style={{ paddingLeft: '20px', margin: 0, color: brand.body, fontSize: '14px', lineHeight: '22px' }}>
          ...
          <li style={{ marginBottom: '10px' }}>
            As soon as {seller} confirms the change in the app, we will let you know by email.
            <Section style={{ marginTop: '8px', padding: '10px 12px', backgroundColor: brand.goldTint, border: `1px solid ${brand.gold}`, borderRadius: '8px' }}>
              <Text style={{ margin: 0, color: brand.ink, fontSize: '13px', lineHeight: '20px' }}>
                💡 Turn on push notifications so you do not miss the moment.{' '}
          ...
          <li style={{ marginBottom: '6px' }}>
            ...
            reference and the name you provided to {seller}. Then, <strong>within 48 hours</strong>,
            head back to the swappup app and confirm everything looks good. Only at that point we will
            send the money to {seller} and your purchase is finalised.
          </li>
        </ol>
      </Text>
      <Heading style={{ ...h1, fontSize: '16px', margin: '24px 0 8px' }}>Found an issue with the booking?</Heading>
      <Text style={p}>
        If something does not look right, or if {seller} does not update the booking with your name
        within 24 hours, your purchase is automatically refunded in full. No forms, no waiting.
      </Text>
